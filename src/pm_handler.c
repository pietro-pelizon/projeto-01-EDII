...
static void processa_comando_p(exhash_t *map, const char *linha_lida) {
    // Declaramos os recipientes aqui dentro!
    char cpf[16], nome[20], sobrenome[20], data_nascimento[12];
    char sexo;

    // O %*s no começo salva a sua vida: ele ignora o 'P'
    sscanf(linha_lida, "%*s %15s %19s %19s %c %11s", cpf, nome, sobrenome, &sexo, data_nascimento);

    // Cria o habitante (isso dá um malloc internamente)
...
static void processa_comando_m(exhash_t *mapa_habitantes, const char *linha_lida) {
    ...
    if (morador != NULL) {
        ...
        printf("Aviso: Comando 'm' falhou. CPF %s nao encontrado no banco.\n", cpf);
    }
}

void pm_processa_arquivo(const char *caminho_arquivo, exhash_t *mapa_habitantes) {
    assert(mapa_habitantes != NULL);
    ...
    while (fgets(linha, sizeof(linha), pm)) {
        ...
        if (strcmp(comandos, "p") == 0) {
            ...
            processa_comando_m(mapa_habitantes, linha);
        }
    }
